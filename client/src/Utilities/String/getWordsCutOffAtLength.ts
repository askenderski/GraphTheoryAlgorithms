function getBiggestFormationUnderLength(stringAsArrOfWordsAndSeparators: string[], maxLength: number) {
...
function getMinAndMaxLengths(text: string, lengthOptions: {minLength: number, maxLength: number}) {
...
function validateLengthOptions(lengthOptions: {minLength: number, maxLength: number}) {
    if (lengthOptions.minLength < 0) {
        throw new Error("Minimum length must be a non-negative number");
    }

    if (lengthOptions.maxLength < 0) {
        throw new Error("Maximum length must be a non-negative number");
    }
}

export const getWordsCutOffAtLength =
    function (text: string, lengthOptions: {minLength: number, maxLength: number}) {
