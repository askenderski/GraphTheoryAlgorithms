import { IAlgorithm, IAdjacencyList } from "Algorithms/IAlgorithm";

const TopSort: IAlgorithm<IAdjacencyList> = {
        ...
        return async function(nodesIds, edgeList) {
            async function dfs(nodeId: string) {
                ...
                for (let i1 = 0; i1 < edgeList.get(nodeId)!.size; i1++) {
                    ...
                    const nextNodeId = edgeList.get(nodeId)!.get(i1)!.get("to");
            ...
            for (let i = 0; i < nodesIds.size; i++) {
                ...
                if (!visited[i]) await dfs(nodesIds.get(i) as string);
                await considerPointerLine(18);
            }
